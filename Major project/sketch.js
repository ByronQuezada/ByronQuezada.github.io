...
class Fighter {
  ...
  display() {
    ...
    image(falling, this.x - 17, this.y - 27, 40, 50);
    if (this.dead === true) {
...
function draw() {
  if (state === 0) {
    ...
    fill(255, 0, 0);
    text ("Hold WASD to fly the background1s \n SPACE bar to shoot", 350, 500);
  }
  else if (state === 1) {
    moveBackground();
    playerPlane();
...
function playerPlane() {
  if (lives > 0) {
    ...
    image(standing, playerX - 20, playerY - 18, 40, 50);
...
function moveBackground() {
  ...
  image(background1,0 , y1, 700, 650);
  image(background1,0 , y2, 700, 650);
